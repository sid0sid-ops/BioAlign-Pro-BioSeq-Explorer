# =====================================================================
# RNA Transcript Server
# =====================================================================
#
# PURPOSE:
#   Manages reactive logic for RNA sequence display:
#   - Tracks zoom level and wrap width settings
#   - Calls transcribe_dna() to convert T→U
#   - Renders formatted RNA sequence with selected visual style

rna_transcript_server <- function(id, shared_state, is_visible = reactive(TRUE), destroy_trigger = NULL) {
  moduleServer(id, function(input, output, session) {
    ...
    register_obs(observe({
      cat("[BioSeq:INFO] RNA Transcript sees sequence of length:", nchar(shared_state$seq_string %||% ""), "\n")
    }))
    ...
    register_obs(observeEvent(shared_state$seq_string, {
      ...
      if (nchar(seq) > 0 && exists("log_sequence_action", mode="function")) {
        log_sequence_action(shared_state, "Generated RNA Transcript")
      }
    }, ignoreInit = TRUE))
    ...
    if (!is.null(destroy_trigger)) {
      observeEvent(destroy_trigger(), {
        ...
        output$rna_render <- NULL
        output$header_badges <- NULL
        output$val_wrap_width <- NULL
      }, ignoreInit = TRUE)
    }

    # Render RNA transcript
    # Pipeline: DNA sequence → transcribe_dna() → format_rna_sequence() → HTML
    output$rna_render <- renderUI({
      req(is_visible())
      ...
      log_message(sprintf("Starting DNA to RNA transcription on sequence scope: %s (%d bp)", seq_name, seq_len))
      ...
      log_message(sprintf("Transcription complete: converted %d bp in %d ms.", seq_len, elapsed_ms))
      
      HTML(format_rna_sequence(rna, wrap_width(), input$visual_style %||% "coloured"))
    })
  })
}
